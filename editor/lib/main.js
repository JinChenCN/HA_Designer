    ...
    $.post("./common/controller.php", { action: 'loadTemp', tempName: tempDiagramName },
        function (data) {
            ...
            try {
                ...
                CONTAINER_MANAGER = ContainerFigureManager.load(obj['p']);
                draw();
                //trigger it to make the options available
                addOptionsForOption_paras();
                //alert("loaded");
            } catch (error) {
                alert("main.js:load() Exception: " + error);
            }
...
/*================= Modified By ZZQ 2015==================== */

function setModelParameterVal(type, objId) {
    var name = "";
    var invariant = "";
    var guard = "";
    var ode_value = "";
    ...
    if (type == "edge") {
        ...
        name = document.getElementById('txtEdgeName').value;
        invariant = document.getElementById('txtAreaEdgeInvariant').value;
        guard = document.getElementById('txtAreaEdgeGuard').value;
        eventv = document.getElementById('txtAreaEdgeEvent').value;
        update = document.getElementById('txtAreaEdgeUpdate').value  ;

        // others = document.getElementById('txtAreaEdgeOthers').value;
    }
    else {
        name = document.getElementById('txtLocationName').value;
        invariant = document.getElementById('txtAreaLocationInvariant').value;
        // guard = document.getElementById('txtAreaLocationGuard').value;
        ode_value = collectOdeStr();
        initv = document.getElementById('txtLocationInit').value;
        isStartLocation = getRadioValue();
        // others = document.getElementById('txtAreaLocationOthers').value;
    ...
    if (type == "edge") {
        for (var i = 0; i < CONNECTOR_MANAGER.connectors.length ; i++) {
            ...
            /* var ConnectMiddleText = obj.middleText;
             if (ConnectMiddleText != null) {
                 /*var str = ConnectMiddleText.str;*/
            var obj = CONNECTOR_MANAGER.connectors[i];
            if (obj.id != objId) {
                continue;
            }
            var ConnectMiddleText = obj.middleText;
            // ConnectMiddleText.str = "name:" + name + ";invariants:" + invariant + ";guards:" + guard;
            var fStr = "name:" + name + ";invariants:" + invariant +
                ";event:" + eventv  
            + ";update:" + update +
                ";guards:" + guard + ";";
            var otherKeyValues = returnNormalFormatByStringOfOthers(others);
            if (otherKeyValues != "") {
                fStr += otherKeyValues;
            }
            // ConnectMiddleText.str = fStr;

            obj.hiddenStr = fStr;
            ConnectMiddleText.str = guard;
        }
        draw();
    ...
    else if (type == "location") //location 
    {
        for (var i = 0; i < STACK.figures.length; i++) {
            ...
            //primitives
            var primitiveList = obj.primitives;
            if (primitiveList.length > 0) {
                for (var pj = 0; pj < primitiveList.length; pj++) {
                    if (primitiveList[pj] != null && primitiveList[pj].oType == "Text") {
                        // primitiveList[pj].str = "name:" + name + ";invariants:" + invariant + ";ode:" + guard;
                        var fstr = "name:" + name + ";invariants:" + invariant 
                        ...
                        /*changed: because the txtbox only show the name of location */
                        primitiveList[pj].str = name;
                        //
                        obj.hiddenStr = fstr;
...
/*
1. str should be the format like odes1:a,b,c;odes2:a,b,c;
2. the return string like "others":{"ode1":["a","b","c"], "odes2":["a","b","c"]},
*/

function returnNormalFormatByStringOfOthers(str) {
    ...
    var items = str.split(";");
    if (items == null || items.lengh < 1) {
        return normalStr;
    }
    var addSpliteSignal = false;
    for (var i = 0 ; i < items.lengh; i++) {
        ...
        var value = tempStrItems[1];
        if (addSpliteSignal) {
            normalStr += ";";
        }
        //append str from the arrays 
        normalStr += tempStr[i];
...
/*
1. str should be the format like odes1:a,b,c;odes2:a,b,c;
2. the return string like "odes1":["a","b","c"],"odes2":["a","b","c"],
*/
function returnJasonFormatByStringOfOthers(str) {
    var jasonStr = "";
    if (str == null || str == "") {
        return jasonStr;
    }
    ...
    // iterate the items and append the strings to the jasonStr
    var addComma = false;
    for (var i = 0 ; i < items.lengh; i++) {
        var tempStr = items[i];
        if (tempStr == "") {
        ...
        var tempStrItems = tempStr.split(":");
        if (tempStrItems == null || tempStrItems.lengh != 2 || tempStrItems[0] == null || tempStrItems[0] == "" || tempStrItems[1] == null || tempStrItems[1] == "") {
            continue;
        }
        var value = tempStrItems[1];
        if (addComma) {
...
b: true, false
*/
function setRadioValue(vi, b)
{
    var startlocations = document.getElementsByName("isstartlocation");

    for (var i = 0; i < startlocations.length; i++) {
        if (startlocations[i].value == vi) {
            startlocations[i].checked = b;
        }
...
function getRadioValue()
{
    ...
    var startlocations = document.getElementsByName("isstartlocation");
    for (var i = 0; i < startlocations.length; i++) {
        if (startlocations[i].checked == true) {
            v = startlocations[i].value;
            break;
        }
...
function showModelParameterVal(type, objId) {

    if (objId == "") {
        if (type == "edge") {

            document.getElementById('txtEdgeName').value = "";
            document.getElementById('txtAreaEdgeInvariant').value = "";
            document.getElementById('txtAreaEdgeGuard').value = "";
            //document.getElementById('txtAreaEdgeOthers').value = "";
            document.getElementById('txtAreaEdgeEvent').value = "";
            document.getElementById('txtAreaEdgeUpdate').value = "";
            

        }
        else {
            document.getElementById('txtLocationName').value = "";
            document.getElementById('txtAreaLocationInvariant').value = "";
            initOde();
            //document.getElementById('txtAreaLocationGuard').value = "";
            //document.getElementById('txtAreaLocationOthers').value = "";
            document.getElementById('txtLocationInit').value = "";
            //document.getElementById('txtAreaLocationGuard').value = "";
            setRadioValue("undefined", "true");
    ...
    if (type == "edge") {
        ...
        data = JSON.parse(data);
        if (data != null && data.edges.length > 0) {
            var length = data.edges.length;
            for (i = 0; i < length; i++) {
                var set = data.edges[i];
                //document.write('<option value="' + set. + '">' + set['name'] + '</option>');
                if (set.id == objId) {

                    document.getElementById('txtEdgeName').value = set.name == null ? "" : set.name;
                    document.getElementById('txtAreaEdgeGuard').value = set.guards == null ? "" : set.guards;
                    document.getElementById('txtAreaEdgeInvariant').value = set.invariants == null ? "" : set.invariants;
                    // document.getElementById('txtAreaEdgeOthers').value = set.invariants == null ? "" : set.invariants;
                    document.getElementById('txtAreaEdgeEvent').value = set.event == null ? "" : set.event;
                    document.getElementById('txtAreaEdgeUpdate').value = set.update == null ? "" : set.update;
    ...
    else if (type == "location") //location 
    {
        ...
        if (data != null && data.locations.length > 0) {
            ...
            for (i = 0; i < length; i++) {
                var set = data.locations[i];
                //document.write('<option value="' + set. + '">' + set['name'] + '</option>');
                if (set.id == objId) {

                    document.getElementById('txtLocationName').value = set.name == null ? "" : set.name;
                    //document.getElementById('txtAreaLocationGuard').value = set.ode == null ? "" : set.ode;
                    initOde(); //clear the original one
                    reRenderOde(set.ode);
                    document.getElementById('txtAreaLocationInvariant').value = set.invariants == null ? "" : set.invariants;
                    document.getElementById('txtLocationInit').value = set.init == null ? "" : set.init;
                    //document.getElementById('txtAreaLocationOthers').value = set.invariants == null ? "" : set.invariants;
                    var isstart = set.isStartLocation == null ? "" : set.isStartLocation;
                    if (isstart == "")
                    { setRadioValue("no", "true"); }
                    else if (isstart == "yes") {
                        setRadioValue("yes", "true");
                    }
                    else { setRadioValue("no", "true"); }
...
function addOptionsForOption_paras() {
    var obj = document.getElementById("option_paras_edges");
    obj.options.length = 0;
    obj.options.add(new Option("-=please select edges=-", ""));
    var data = returnModelInfoInJasonFormat();
    ...
    if (data != null && data.edges.length > 0) {
        ...
        for (i = 0; i < length; i++) {
            ...
            str = " " + set.startLocation.showname + "->" + set.endLocation.showname;
            obj.options.add(new Option(str, set.id));
        }
    }

    obj = document.getElementById("option_paras_locations");
    obj.options.length = 0;
    obj.options.add(new Option("-=please select locations=-", ""));
    if (data != null && data.locations.length > 0) {
        var length = data.locations.length;
        for (i = 0; i < length; i++) {
            var set = data.locations[i];
            ...
            //str = " " + set.startLocation.text + "->" + set.endLocation.text;
            obj.options.add(new Option(set.showname, set.id));
        }
    }
...
/**Triggered when click the button of Export Model Info.
Returns a Jason String 
 **/

function returnModelInfoInJasonFormat() {
    /*
    example: 
    { "modelName": "abc", 
    "edges":[{"id":"1","events":["abc",""],"guards":["123","345"], "startLocation":{"id":"1","name":"1", "text":"abc"}, "endLocation":{"id":"2", "text":"abcd"} }, 
             {"id":"2","events":["abc",""],"guards":["123","345"], "startLocation":{"id":"3", "name":"1","text":"abc"}, "endLocation":{"id":"4", "text":"abcd"} }
            ], 
    "locations": [
    {"id":"1", "text":"abc"},
    {"id":"2", "text":"abc"},
    ...
    var outputJason = "{ \"modelName\": \"" + document.getElementById("txtModelName").value + "\","; //"{\"modelName\": \"" + document.getElementById("txtModelName").value + "\",}";
    // start edges 
    outputJason += "\"edges\":[";
    ...
    for (var i = 0; i < CONNECTOR_MANAGER.connectors.length ; i++) {
        ...
        var startFigure = STACK.figureGetById(CONNECTOR_MANAGER.connectionPointGetById(CONNECTOR_MANAGER.connectionPointGetByXY(obj.turningPoints[0].x, obj.turningPoints[0].y, "figure")).parentId);
        var endFigure = STACK.figureGetById(CONNECTOR_MANAGER.connectionPointGetById(CONNECTOR_MANAGER.connectionPointGetByXY(obj.turningPoints[1].x, obj.turningPoints[1].y, "figure")).parentId);
        edgeStr += ",\"startLocation\":{\"id\":\"" + startFigure.id + "\", \"showname\":\"" + returnFigureShowName(startFigure) + "\", \"text\":\"" + returnFigureText(startFigure) + "\"}";
        edgeStr += ",\"endLocation\":{\"id\":\"" + endFigure.id + "\",\"showname\":\"" + returnFigureShowName(endFigure) + "\",  \"text\":\"" + returnFigureText(endFigure) + "\"}";
        //primitives
        var ConnectMiddleText = obj.middleText;
        if (ConnectMiddleText != null && ConnectMiddleText.str != "") {
            /*var str = ConnectMiddleText.str;
            var strs = str.split(",");
            if (strs != null && strs.length == 2) {
                edgeStr += ",\"guards\":\"" + strs[0] + "\"";
                edgeStr += ",\"events\":\"" + strs[1] + "\"";
            }*/

            // edgeStr += returnConditionJasonStrByText(ConnectMiddleText.str);

            //obj.hiddenStr is not the latest txt 
            var ts = ConnectMiddleText.str;
            if (obj.hiddenStr != null && obj.hiddenStr != "") {
                var m = obj.hiddenStr.indexOf("guards:");
                if (m != -1) {

                    var j = obj.hiddenStr.indexOf(";", m);
                    if (j != -1) {
                        obj.hiddenStr = obj.hiddenStr.substring(0, m + 7) + ts + obj.hiddenStr.substring(j);
                    }


                }
                else {
                    obj.hiddenStr = "guards:" + ts + ";";
                }
            }

        }
        edgeStr += returnConditionJasonStrByText(obj.hiddenStr);
    ...
    for (var i = 0; i < STACK.figures.length; i++) {
        ...
        locationStr += "{\"id\":\"" + obj.id + "\", \"showname\":\"" + returnFigureShowName(obj) + "\", \"text\":\"" + returnFigureText(obj) + "\"";
        //add name, guard,invariant 
        var txtString = obj.hiddenStr;
        if (txtString != null) {
            /*
            var paras = txtString.split(",");
            for (var j = 0 ; j < paras.length; j++)
            {
                var tStr = paras[j];
                if (tStr != null)
                {
                    var keyValue = tStr.split(":");
                    if (keyValue.length == 2)
                    {
                        locationStr += ",\"" + keyValue[0] + "\":\"" + keyValue[1] + "\"";;
                    }
                }
            }
            */
...
function captureInterfaceValues() {
    ...
    values += collectExternalVarableStr();
...
function returnConditionJasonStrByText(txtString) {
    ...
    if (txtString != null) {
        ...
        for (var j = 0 ; j < paras.length; j++) {
            ...
            if (tStr != null && tStr != "") {
                var keyValue = tStr.split(":");
                if (keyValue != null && keyValue.length > 0 && keyValue.length != 2 && keyValue[0] == "ode")
                {
                    var s = tStr.substring(5, tStr.length-1);
                    s = s.replace(/'/g, "\"");
                    str += "," + s;
                    
                }
                else if (keyValue.length == 2 && keyValue[0] != "ode") {
                    //str += ",\"" + keyValue[0] + "\":\"" + keyValue[1] + "\"";
                    str += ",\"" + keyValue[0] + "\":";
                    //followed by : []
                    str += "[";
                    // splite the items into array by comma
                    var itemsList = keyValue[1].split(",");
                    if (itemsList != null && itemsList.length > 0) {
                        for (var f = 0; f < itemsList.length; f++) {
                            str += "\"" + itemsList[f] + "\"";
                            if (f != itemsList.length - 1) {
                                str += ",";
                            }
                        }
                    }
...
function returnFigureShowName(figureObj) {
    var obj = figureObj;
    var primitiveList = obj.primitives;
    var str = "";
    if (primitiveList.length > 0) {
...
function returnFigureText(figureObj) {
    var obj = figureObj;
    var primitiveList = obj.primitives;
    var str = "";
    if (primitiveList.length > 0) {
    ...
    /*return str;*/
    //primitiveList[pj].str = name
    //replace the hiddenStr : "name:" + name + ";
    var ts = obj.hiddenStr; //.replace(/name:\s*;/g, "name:" + str);
    var na = "";
    var i = ts.indexOf("name:")
    if (i != -1) {
        var j = ts.indexOf(";", i);
        if (j != -1) {
            ts = ts.substring(0, i + 5) + str + ts.substring(j);
        }
    }
    else {
    ...
    obj.hiddenStr = ts;
    return obj.hiddenStr;//== undefined || obj.hiddenStr == "" ? str : obj.hiddenStr;
}

function returnStackFigureLength() {
    var strs = "";
    for (var i = 0; i < STACK.figures.length; i++) {
        var obj = STACK.figures[i];
        ...
        if (primitiveList.length > 0) {
            for (var pj = 0; pj < primitiveList.length; pj++) {
                if (primitiveList[pj] != null && primitiveList[pj].oType == "Text") {
                    strs = strs + ("\n" + obj.name + "---id" + obj.id + "-----text:" + primitiveList[pj].str);
                }
            }
        }
    ...
    //alert("22222");
    alert(strs);
    returnConnectorLength();
    //var SaveTextAs = new saveTextAs();
    //saveTextAs(strs, "test.html");

}

function returnConnectorLength() {
    var strs = "";

    /* linkMap() here is empty*/
    //alert("link map:" + linkMap());

    //to show how many connectors in the picture
    alert("Connector length:" + CONNECTOR_MANAGER.connectors.length);

    //iterater the connectors in the picture
    for (var i = 0; i < CONNECTOR_MANAGER.connectors.length ; i++) {
        var obj = CONNECTOR_MANAGER.connectors[i];

        alert("from: " + CONNECTOR_MANAGER.connectionPointGetById(CONNECTOR_MANAGER.connectionPointGetByXY(obj.turningPoints[0].x, obj.turningPoints[0].y, "figure")).parentId
                + " \n to: " + CONNECTOR_MANAGER.connectionPointGetById(CONNECTOR_MANAGER.connectionPointGetByXY(obj.turningPoints[1].x, obj.turningPoints[1].y, "figure")).parentId
                );
        //primitives
        var ConnectMiddleText = obj.middleText;

        if (ConnectMiddleText != null) {
            strs = strs + ("\n" + obj.oType + "---id" + obj.id + "-----text:" + ConnectMiddleText.str);

            //alert("start points:" + obj.turningPoints[0]);
            //get the first point from the connector
            var firstConPoint = CONNECTOR_MANAGER.connectionPointGetFirstForConnector(obj.id);
            //try to get all the figures from the first point of connector, nothing returns. 
            //var figuress = STACK.figuresGetByXY(obj.turningPoints[0].x, obj.turningPoints[0].y);


            //var figuress = STACK.figuresGetByXY(202, 73);

            var gluesArray = CONNECTOR_MANAGER.glues;
            if (gluesArray != null && gluesArray.length > 0) {
                // get glue[i]. id1
                for (var gIndex = 0; gIndex < gluesArray.length; gIndex++) {

                    //alert(gluesArray[gIndex].id1);
                    /*    alert("connectionId:" + gluesArray[gIndex].id1 + "'s parentId:" + CONNECTOR_MANAGER.connectionPointGetById(gluesArray[gIndex].id1).parentId
                            +"\n" +
                             "connectionId:" + gluesArray[gIndex].id2 + "'s parentId:" + CONNECTOR_MANAGER.connectionPointGetById(gluesArray[gIndex].id2).parentId
                            ); */

                }
                // through id1 to get points and then get the parentId

            }

            if (firstConPoint != null) {

                // alert("start points:" +  firstConPoint.parentId);
            }
            var secConPoint = CONNECTOR_MANAGER.connectionPointGetSecondForConnector(obj.id);
            if (secConPoint != null) {
                //  alert("start points:" + secConPoint.parentId);
            }
        }
...
function loadModelFromLocal(data) {
    //alert("load diagram [" + diagramId + ']');
